    ...
    >
      ...
      <div 
        className={`px-4 py-2 border-b transition-colors ${
          activeStrategy.id === strategy.id ? 'bg-opacity-10' : ''
        }`}
        style={{ 
          borderColor: 'var(--border)',
          backgroundColor: activeStrategy.id === strategy.id ? 'rgba(59, 130, 246, 0.1)' : 'transparent'
        }}
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="flex items-center space-x-2">
              <BarChart3 
                size={16} 
                style={{ color: activeStrategy.id === strategy.id ? 'var(--accent)' : 'var(--text-secondary)' }} 
              />
              <h3 
                className="font-semibold text-sm"
                style={{ color: activeStrategy.id === strategy.id ? 'var(--accent)' : 'var(--text-primary)' }}
              >
      ...
      <div className="p-4">
        <div className="relative">
          {/* Chart Container */}
          <div 
            className="h-48 rounded border flex items-center justify-center relative overflow-hidden"
            style={{
              backgroundColor: 'var(--bg-primary)',
              borderColor: 'var(--border)'
            }}
